...
export default function TrainingEnrollPage({ searchParams }: { searchParams: { [key: string]: string | undefined } }) {
  const planId = searchParams.plan || "basic";
  const onlinePlan = trainingContent.in.online.find((p) => p.id === planId);
  const offlinePlan = trainingContent.in.offline.find((p) => p.id === planId);
  
  const plan = onlinePlan || offlinePlan;

  if (!plan) {
    return (
      <div className="min-h-screen flex items-center justify-center dark:bg-gray-900 text-gray-500">
        Invalid Plan Selected. <Link href="/training" className="text-purple-600 ml-2">Go back</Link>
      </div>
    );
  }
  ...
  return (
    <main className="relative flex flex-col items-center justify-center min-h-[100dvh] overflow-hidden bg-[#f8f9fc] dark:bg-gray-900">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <div className="absolute top-0 left-0 w-full h-48 bg-gradient-to-b from-purple-100/50 to-transparent dark:from-purple-900/20"></div>
        <div className="absolute top-[-20%] right-[-10%] w-[50%] h-[50%] bg-green-200/40 dark:bg-green-900/20 rounded-full mix-blend-multiply filter blur-[100px] opacity-60"></div>
      </div>
      ...
      <div className="z-10 w-full max-w-sm mx-auto px-4 py-8 flex flex-col h-[100dvh]">
        ...
        <div className="flex justify-between items-center mb-6">
          <Link href="/training" className="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors flex items-center gap-1 text-sm font-medium">
        ...
        <div className="flex-grow flex flex-col justify-center">
           ...
           <TrainingPayment title={plan.title} amount={plan.price} />
        </div>
